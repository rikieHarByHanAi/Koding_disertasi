...
# 
# Versi      : 4.1 — Fix NaN α (prev_losses init + robust aggregation)
# ============================================================
#
...
BETA_1 = 0.4    # bobot akurasi (Acc̃)   — stabilitas deteksi
BETA_2 = 0.4    # bobot penurunan loss (ΔL̃) — arah konvergensi
BETA_3 = 0.2    # bobot loss sesaat (1-L̃)   — kualitas instan
# β1+β2+β3 = 1.0 ✓
assert abs(BETA_1 + BETA_2 + BETA_3 - 1.0) < 1e-9, "β1+β2+β3 harus = 1"

# ── Hyperparameter FL ────────────────────────────────────────
N_CLIENTS       = 5       # jumlah controller SDN
T_MAX           = 200     # batas maksimum round (Kondisi 2 Bab III)
LOCAL_EPOCHS    = 5       # epoch lokal per round
BATCH_SIZE      = 64
LR_CLIENT       = 1e-3

# ── Kriteria konvergensi (Persamaan 3.6) ────────────────────
DELTA           = 1e-4   # δ — ambang batas perubahan loss global
TAU             = 5      # τ — jumlah round berturut-turut untuk konvergensi

# ── Early stopping berbasis val accuracy ────────────────────
PATIENCE        = 20     # round tanpa peningkatan val acc

# ── Konstanta numerik (Persamaan 3.3–3.5) ───────────────────
EPSILON         = 1e-8   # ε — mencegah pembagian nol

# ── Non-IID ─────────────────────────────────────────────────
DIRICHLET_ALPHA = 0.5    # α Dirichlet
TEST_RATIO      = 0.20
...
# Held-out test set global — tidak disentuh sampai evaluasi akhir
...
def compute_dts(acc_list, loss_list, prev_loss_list,
    ...
    Args:
        acc_list      : [Acc_k^t] akurasi lokal tiap client round t
        loss_list     : [L_k^t]   loss lokal tiap client round t
        prev_loss_list: [L_k^{t-1}] loss round sebelumnya.
                        None = round pertama, ΔL ← 0 (Algoritma 1 Bab III)
    Returns:
        T_k^t (array, satu nilai per client)
    """
    acc_arr  = np.array(acc_list,  dtype=np.float64)
    loss_arr = np.array(loss_list, dtype=np.float64)

    # ΔL_k^t = L_k^{t-1} - L_k^t  (Persamaan 3.4)
    # Untuk round pertama (t=1): ΔL¹_k ← 0  (Algoritma 1 Bab III Langkah 1)
    if prev_loss_list is None:
        delta_loss = np.zeros_like(loss_arr)
    else:
        prev_arr   = np.array(prev_loss_list, dtype=np.float64)
        delta_loss = prev_arr - loss_arr   # positif = perbaikan model

    # Normalisasi min-max (Persamaan 3.3, 3.4, 3.5)
    acc_norm   = minmax_normalize(acc_arr,    eps)   # Acc̃
    delta_norm = minmax_normalize(delta_loss, eps)   # ΔL̃
    loss_norm  = minmax_normalize(loss_arr,   eps)   # L̃
...
def dts_aggregate(client_weights_list, client_sizes, trust_scores):
    ...
    Properti: Σ α_k^t = 1 dan α_k^t ≥ 0
    Fallback : jika Σ(T·n) ≈ 0 atau NaN → FedAvg biasa (bobot n_k)
               Ini terjadi hanya di round pertama saat semua DTS identik.
    """
    ...
    if np.isnan(total) or total < EPSILON:
        # Fallback ke FedAvg: bobot proporsional n_k saja
        alpha = n / n.sum()
    else:
        alpha = Tn / total

    # Clip dan renormalisasi untuk menjamin numerik stabil
    alpha = np.clip(alpha, 0.0, 1.0)
    alpha = alpha / alpha.sum()   # Σ α_k^t = 1 dijamin

    # Agregasi berbobot (Persamaan 3.1)
    aggregated = []
...
# Inisialisasi (Algoritma 1, Langkah 1)
global_model   = build_model(n_features)
global_weights = get_weights(global_model)

# prev_losses = None → round pertama ΔL¹_k ← 0 (Algoritma 1 Bab III)
prev_losses    = None

history = {
    'round':       [],
    'train_loss':  [],  # rata-rata loss lokal
    'train_acc':   [],
    'val_acc':     [],
    'val_f1':      [],
    'global_loss': [],  # L^t_global = (1/K) Σ L_k^t
    'trust_scores': [],  # T_k^t per round
    'alpha':       [],   # α_k^t per round
}
...
# Kriteria konvergensi (Persamaan 3.6)
cnt_conv        = 0          # cntconv
prev_global_loss = np.inf    # L^{t-1}_global

actual_rounds   = 0
stopped_by      = "T_max"

# Header print — dinamis sesuai N_CLIENTS
t_header  = ' '.join(f'T{k+1:>4}' for k in range(N_CLIENTS))
a_header  = ' '.join(f'α{k+1:>4}' for k in range(N_CLIENTS))
col_width = 4 + 11 + 9 + 9 + 8 + 3 + N_CLIENTS*6 + 3 + N_CLIENTS*6
...
for t in range(1, T_MAX + 1):
    ...
    # ── Langkah 9: ΔL — round pertama ΔL=0, selanjutnya L^{t-1}-L^t ──
    # ── Langkah 10–18: Normalisasi & Hitung DTS ─────────────────
    trust_scores = compute_dts(round_accs, round_losses, prev_losses)

    # ── Langkah 19: Hitung α_k^t ────────────────────────────────
    # ── Langkah 20: Agregasi global ─────────────────────────────
    global_weights, alpha = dts_aggregate(all_weights, all_sizes, trust_scores)
    set_weights(global_model, global_weights)

    # ── Langkah 21: L^t_global = (1/K) Σ L_k^t ─────────────────
    global_loss_t = float(np.mean(round_losses))

    # Simpan L_k^t sebagai L_k^{t-1} untuk round berikutnya (prev_losses: None→list)
    prev_losses = round_losses.copy()

    # ── Langkah 22–26: Cek konvergensi (Persamaan 3.6) ──────
    ...
    # ── Evaluasi validasi (tiap 5 round untuk efisiensi) ────
    if t % 5 == 0 or t == 1:
        val_accs, val_f1s = [], []
        for Xvl, yvl in client_vals:
            yhat = (global_model.predict(
                Xvl.astype(np.float32), verbose=0) > 0.5).astype(int).flatten()
    ...
    # ── Langkah 27–29: Cek konvergensi τ round ──────────────
    if cnt_conv >= TAU:
        stopped_by = f"Konvergensi loss global (τ={TAU} round, δ={DELTA})"
        break

    # ── Langkah 30–32: Batas T_max ──────────────────────────
    if t >= T_MAX:
        stopped_by = f"T_max={T_MAX} tercapai"
        break

    # ── Langkah 33: Distribusi model global ─────────────────
    # (dilakukan otomatis: global_weights sudah diperbarui)
...
fedavg_best_acc = 0.0
fedavg_prev_losses = None      # sama dengan DTS: round pertama ΔL=0
fedavg_cnt_conv = 0
...
for i, (Xvl, yvl) in enumerate(client_vals):
    ...
    name = f"Ctrl_{i+1}"
    per_ctrl[name] = {
        'accuracy':   accuracy_score(yvl, yhat),
        'precision':  precision_score(yvl, yhat, zero_division=0),
        'recall':     recall_score(yvl, yhat, zero_division=0),
        'f1':         f1_score(yvl, yhat, zero_division=0),
        'attack_pct': np.mean(yvl) * 100,
        'n_samples':  len(yvl),
...
# ── Plot 2: Val Accuracy — DTS vs FedAvg ─────────────────
...
trust_arr = np.array(history['trust_scores'])   # shape: (n_eval, N_CLIENTS)
...
# ── Plot 4: Bobot Agregasi α per controller ──────────────────
...
print(f"""
{'='*65}
RINGKASAN HASIL — DISERTASI & JURNAL JINITA SINTA 2
Federated Learning dengan Dynamic Trust Score Aggregation
{'='*65}
